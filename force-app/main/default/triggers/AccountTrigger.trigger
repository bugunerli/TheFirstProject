...
trigger AccountTrigger on Account (before insert, after insert, 
     ...
     // Trigger.new Context variable, hold the List<Account> that entered teh Trigger
     ...
     if(Trigger.isBefore && Trigger.isInsert){
        ...
        for(Account each : Trigger.new) {
         ...
         // NO DML Needed in BeforeInsert, because the next thing after 
     }
     ...
     if(Trigger.isAfter && Trigger.isInsert){
          ...
          // so we need a List<Contact> to store it and eventually insert this using DML outside the loop 
